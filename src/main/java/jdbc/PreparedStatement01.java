...
// bir tablonun istenilen datasını prepared statemnt ile çagırtmak ıcın kullanılan metod
    public static void Read_data(Connection con,String tableName) throws SQLException {



        try {
            String query=String.format("select * from %s",tableName);// format() metodu dınamık strıng olusturmak için kullanılır
            Statement statement= con.createStatement();

            // SQL query calıstır.
            ResultSet rs=statement.executeQuery(query);// datayı cagırıp result set konteynırına koyuyoruz

            while (rs.next()){// tum datayı cagıralım
